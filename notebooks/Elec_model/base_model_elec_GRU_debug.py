...
def train_model(model, 
    ...
    for _ in range(params["target_seq_length"]):
        with torch.no_grad():
            next_step_forecast, _ = model(last_sequence) # [1, 16]
...
for i, train_df_slice in enumerate(train_df_list):
    ...
    loss = smape_loss(torch.from_numpy(forecast_seq_descaled[:,-1].reshape(-1, 1)), torch.from_numpy(test_df_list[i].to_numpy().reshape(-1, 1)))

    print(f'The SMAPE loss for {i}: {loss.item()}')
