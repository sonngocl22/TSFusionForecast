...
x_train, y_train = get_x_y(
        ...
        target_sequence_length=params["target_sequence_length"],
        input_seq_len=params["in_length"]
        )

def objective(trial):

    hyperparameters = {
        'num_leaves': trial.suggest_int('num_leaves', 5, 25, step=1),
        'min_data_in_leaf': trial.suggest_int('min_data_in_leaf', 5, 25, step=1),
    ...
    x_test = train_val_dict['train_set'][feature_variable].iloc[-params["in_length"]:].to_numpy().reshape(1, -1)
...
if os.path.exists(os.path.join(elec_dir, 'best_params', 'best_params_lgb.pkl')):
    print('Loading best params...')
    with open(os.path.join(elec_dir, 'best_params', 'best_params_lgb.pkl'), 'rb') as fin:
    ...
    with open(os.path.join(elec_dir, 'best_params', 'best_params_lgb.pkl'), 'wb') as fout:
...
y_hat_df.to_csv(os.path.join(elec_dir, 'base_models_ts', 'y_hat_df_lgb.csv'), index=False)
