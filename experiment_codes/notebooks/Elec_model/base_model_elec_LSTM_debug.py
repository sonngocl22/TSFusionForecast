...
#     data_X = df[feature_variable].values
#     data_y = df[target_variable].values.reshape(-1, 1)
#     data_X, scaler_X = normalize_data(data_X)
#     data_y, scaler_y = normalize_data(data_y)
#     X, Y = [], []
#     sequences_dict = {}

#     # price_de_idx = df.columns.get_loc('price_de')

#     for i in range(len(data_X) - seq_length - target_seq_length):
#         x = data_X[i:(i + seq_length)]
#         # y = data[i + seq_length]
#         y = data_y[(i + seq_length):(i + seq_length+target_seq_length)].flatten()

#         X.append(x)
#         Y.append(y)

#     sequences_dict = {'X' : np.array(X), 'y': np.array(Y), 'scaler_X' : scaler_X, 'scaler_y' : scaler_y}
...
def train_model(model, 
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            # print(smape_loss(torch.from_numpy(y_val).to(device), model(X_train[-1:].to(device)).flatten()).item())
...
def objective(trial):
    ...
    hidden_size = trial.suggest_int("hidden_size", 10, 60, step=5)
    ...
    # applying inverse transform to the forecast based on the last column of the scaler
    # forecast_seq_descaled = (forecast_seq.detach().cpu().numpy() / sequences_dict['scaler'].scale_[-1]) + sequences_dict['scaler'].min_[-1]

    # print(forecast_seq.detach().cpu().numpy())
    ...
    # loss = criterion(torch.from_numpy(forecast_seq.detach().cpu().numpy().flatten()), torch.from_numpy(train_val_dict['val_set'].iloc[:,-1].values))
...
# loss_smape = smape_loss(torch.from_numpy(forecast_seq_descaled.flatten()), torch.from_numpy(test_df_list[0].to_numpy().flatten()))
...
y_hat_df.to_csv(os.path.join(elec_dir, 'base_models_ts', 'y_hat_df_lstm_test_bm14.csv'), index=False)
