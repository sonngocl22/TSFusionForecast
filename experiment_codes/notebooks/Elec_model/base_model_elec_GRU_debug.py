...
def train_model(model, 
    ...
    data_loader = DataLoader(dataset, batch_size=batch_size, shuffle=False, drop_last=True) # droping the last batch to avoid errors

    for epoch in range(epochs):
        model.train()
        h0 = None  # Initialize h0 at the beginning of each epoch

        for batch_idx, (sequences, targets) in enumerate(data_loader):
            sequences, targets = sequences.to(device), targets.to(device)

            optimizer.zero_grad()
            pred, h0 = model(sequences, h0)
            h0 = h0.detach()  # Detach h0 from the graph
    ...
    forecast_seq, _ = model(last_sequence)
...
def objective(trial):
    ...
    print(smape_loss(forecast_seq_descaled.flatten(), train_val_dict['val_set'].iloc[:,-1].values))
...
train_df_slice = train_df_list[-1]
...
y_hat_df.to_csv(os.path.join(elec_dir, 'base_models_ts', 'y_hat_df_gru_bm14_test.csv'), index=False)
