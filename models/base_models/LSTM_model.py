...
class LSTMModel(BaseModel):
    ...
    def _tuning_objective(self, trial, train_data, validation_data):
        ...
        """
        Objective function for tuning the LSTM model. The hyperparameters are tuned using Optuna.
        """

        learning_rate = trial.suggest_float("learning_rate", *self.configs.LSTM_TUNING_PARAMS['learning_rate'], log=True)
        hidden_size = trial.suggest_int("hidden_size", *self.configs.LSTM_TUNING_PARAMS['hidden_size'], step=5)
        num_layers = trial.suggest_int("num_layers", *self.configs.LSTM_TUNING_PARAMS['num_layers'])
        batch_size = trial.suggest_categorical("batch_size", self.configs.LSTM_TUNING_PARAMS['batch_size'])
        dropout = trial.suggest_float("dropout", *self.configs.LSTM_TUNING_PARAMS['dropout'])
        ...
        model = _LSTM_Model(len(self.feature_variable), hidden_size, num_layers, self.config.GLOBAL_PARAMS['target_sequence_length'], dropout)
        model.to(device)
        ...
        _, forecast_seq = self._train_model(model,
                                            ...
                                            epochs=self.configs.LSTM_TUNING_PARAMS['train_epochs'],)
    ...
    def train(self, retune=False, best_params_path="models/base_models/best_params"):
        ...
        if not retune:
            ...
            assert os.path.isfile(os.path.join(best_params_path, 'best_params_lstm.pkl')), "No best params file found. Please run tuning first."
            with open(os.path.join(best_params_path, 'best_params_lstm.pkl'), 'rb') as fin:
            ...
            study.optimize(lambda trial: self._tuning_objective(trial, 
                                         ...
                                         n_trials=self.configs.LSTM_TUNING_PARAMS["n_trials"], 
                                         show_progress_bar=True)
            best_params = study.best_trial.params
            with open(os.path.join(best_params_path, 'best_params_lstm.pkl'), 'wb') as fout:
        ...
        model = _LSTM_Model(len(self.feature_variable), best_params['hidden_size'], best_params['num_layers'], self.config.GLOBAL_PARAMS['target_sequence_length'], best_params['dropout'])
        model.to(device)

        criterion = nn.MSELoss()
        optimizer = torch.optim.Adam(model.parameters(), lr=best_params['learning_rate'])

        self.model_lstm, forecast_seq = self._train_model(model,
                                                         criterion=criterion,
                                                         optimizer=optimizer,
                                                         X_train=X_train,
                                                         y_train=y_train,
                                                         batch_size=best_params['batch_size'],
                                                         epochs=configs.LSTM_TUNING_PARAMS['train_epochs'],)
    ...
    def generate_meta(self, meta_data_path="models/meta_data"):
        ...
        for i, train_df_slice in enumerate(self.train_df_slices):
            ...
            print(f'The LSTM SMAPE loss for {i} slice: {loss_smape.item()}')
        ...
        y_hat_df.to_csv(os.path.join(meta_data_path, f'bm{self.configs.GLOBAL_PARAMS["meta_data_len"]}', f'y_hat_df_lstm_bm{self.configs.GLOBAL_PARAMS["meta_data_len"]}.csv'), index=False)
    ...
    def save_model(self, file_path="models/base_models/saved_models"):
        ...
        torch.save(self.model_lstm.state_dict(), os.path.join(file_path, f'lstm_model_bm{self.configs.GLOBAL_PARAMS["meta_data_len"]}.pth'))
    ...
    def load_model(self, file_path="models/base_models/saved_models", best_params_path="models/base_models/best_params"):
        ...
        assert os.path.isdir(file_path), f"Directory {file_path} does not exist."
        assert os.path.isfile(os.path.join(file_path, f'lstm_model_bm{self.configs.GLOBAL_PARAMS["meta_data_len"]}.pt')), "No model file found. Please run training first."
        ...
        self.model_lstm = _LSTM_Model(len(self.feature_variable), best_params['hidden_size'], best_params['num_layers'], self.config.GLOBAL_PARAMS['target_sequence_length'], best_params['dropout'])
        self.model_lstm.load_state_dict(torch.load(os.path.join(file_path, f'lstm_model_bm{self.configs.GLOBAL_PARAMS["meta_data_len"]}.pt')))
